vim.pack.add({
    { src = "https://github.com/saghen/blink.cmp", name = "blink.cmp", rev = "v1.10.2" },
})
...
blink.setup({
    ...
    keymap = {
        ...
        ["<C-e>"] = { "hide", "fallback" },
        -- With preselect off, plain "accept" has no selection → falls through to
        -- "fallback" and inserts a newline. "select_and_accept" picks the first item
        -- when none is highlighted (VS Code–style). LSP import edits still apply on accept.
