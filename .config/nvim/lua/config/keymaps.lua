...
-- theme
vim.keymap.set('n', '<leader>sc', '<cmd>Telescope colorscheme<CR>', { desc = 'Select colorscheme' })

-- Clear highlights on search when pressing <Esc> in normal mode
vim.keymap.set('n', '<Esc>', '<cmd>nohlsearch<CR>', { desc = 'Clear search highlights' })

-- Packages
vim.keymap.set('n', '<leader>lpu', '<cmd>lua vim.pack.update()<CR>', { desc = 'Update nvim plugins' })
vim.keymap.set('n', '<leader>lpd', function()
    vim.ui.input({ prompt = 'Delete plugin: ' }, function(input)
        if input and input ~= "" then
            -- vim.pack.del expects a list of plugin names/srcs
            local status, err = pcall(function()
                vim.pack.del({ input })
            end)

            if status then
                vim.notify("Deleted plugin: " .. input, vim.log.levels.INFO)
            else
                vim.notify("Error deleting plugin: " .. tostring(err), vim.log.levels.ERROR)
            end
        end
    end)
end, { desc = 'Delete a plugin via vim.pack' })
